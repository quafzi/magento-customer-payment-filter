 ...
 */
class Quafzi_CustomerPaymentFilter_AdminController extends Mage_Adminhtml_Controller_Action
{
		...
		if(false == is_array($customerIds)) {
			...
			try {
				foreach($customerIds as $customerId) {
                    ...
                    $deniedMethods = $this->getRequest()->getParam('denied_payment_methods');
                    $deniedMethods = implode(',', $deniedMethods);
                    $customer->setDeniedPaymentMethods($deniedMethods);
                    $customer->getResource()->saveAttribute($customer, 'denied_payment_methods');
				}
